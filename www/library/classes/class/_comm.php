...
class class_comm extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function sendEmail($recepient, $template, $receiver = array()) {
		...
		$file = $_SERVER['DOCUMENT_ROOT'].$template['template_file'];
		...
		$html = str_replace('[browser]', '<a style="text-decoration: none;color: black;" href="http://[host]/mailer/view/'.$data['_comm_code'].'">View mail on browser</a>', $html);
		$html = str_replace('[unsubscribe]', '<a style="text-decoration: none;" href="http://[host]/mailer/unsubscribe/'.$data['_comm_code'].'">Click to unsubscribe</a>', $html);
		$html = str_replace('[track]', '<img src="http://[host]/mailer/tracking/'.$data['_comm_code'].'" width="0" height="0" border="0"  />', $html);
