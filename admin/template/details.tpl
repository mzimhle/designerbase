...
<div class="container">
  <div class="content">
	...
	<ol class="breadcrumb">
		...
		<li>{if isset($templateData)}{$templateData.template_code}{else}Add a template{/if}</li>
          ...
          <div class="portlet">
            <div class="portlet-header">
              <h3>
				...
				{if isset($templateData)}{$templateData.template_code}{else}Add a template{/if}
              </h3>
            </div> <!-- /.portlet-header -->
            <div class="portlet-content">
              <form id="validate-basic" action="/template/details.php{if isset($templateData)}?code={$templateData.template_code}{/if}" method="POST" data-validate="parsley" class="form parsley-form" enctype="multipart/form-data">
                ...
                <div class="form-group">
					...
					<input type="text" id="template_code" name="template_code" class="form-control" data-required="true" value="{$templateData.template_code}" />
					{if isset($errorArray.template_code)}<span class="error">{$errorArray.template_code}</span>{else}<span class="smalltext">code of the template</span>{/if}					  
                </div>
                <div class="form-group">
					...
					<select id="template_type" name="template_type" class="form-control">
						...
						<option value="CHECKOUT" {if $templateData.template_type eq 'CHECKOUT'}selected{/if}> CHECKOUT </option>
					</select>
