...
$imageObject	= new File(array('jpg', 'jpeg', 'png', 'gif'));
...
if(count($_POST) > 0) {
	...
	if(isset($_POST['template_code']) && trim($_POST['template_code']) == '') {
		$errorArray['template_code'] = 'Please add a code';	
	} else {
		$code = isset($templateData) ? $templateData['template_code'] : null;
		
		$checkData = $templateObject->checkCode(trim($_POST['template_code']), $code);

		if($checkData) {
			$errorArray['template_code'] = 'Code selected already exists';	
		}
	}
	...
	if(isset($_POST['template_category']) && trim($_POST['template_category']) != '') {
		...
		if($category == 'EMAIL') {
			if(isset($_POST['template_subject']) && trim($_POST['template_subject']) == '') {
			...
			if(!isset($_POST['template_message'])) {
				...
				$errorArray['template_message'] = 'Please add a message';	
			} else if(strlen(trim($_POST['template_message'])) > 140) {
				$errorArray['template_message'] = 'Message needs to be less than 140 characters';	
			}
		} else {
			$errorArray['template_category'] = 'No category has been selected';	
		}
	} else {
		$errorArray['template_category'] = 'No category has been selected';	
	}
					...
					if(isset($_FILES['imagefiles']) && count($_FILES['imagefiles']['name']) > 0) {
						for($i = 0; $i < count($_FILES['imagefiles']['name']); $i++) {
							if($_FILES['imagefiles']['name'][$i] != '') {
								$directory	= $zfsession->config['path'].'\media\template\\'.strtolower($success).'\images\\';
