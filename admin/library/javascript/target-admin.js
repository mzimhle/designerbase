...
$(function () {
  target_admin.init ();
	...
	if($( "#location_pathname" ).length) {
		$( "#location_pathname" ).autocomplete({
				source: "/feeds/location.php",
				minLength: 2,
				select: function( event, ui ) {
					...
					if(ui.item.id == '') {
						...
						$('#location_pathname').html('<b>' + ui.item.value + '</b>');
						$('#location_code').val(ui.item.id);	
					}
					$('#location_pathname').val('');										
				}
			});
	}
