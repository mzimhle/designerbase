				...
				<div class="col-sm-6">
					<div class="form-group">
						...
						<select class="form-control" id="filter_cipher" name="filter_cipher">
							<option value=""> -----  </option>
							{html_options options=$selectCipher}						
						</select>
					</div>					
					<div class="form-group">
				...
				<form id="validate-basic" action="/dashboard/feature/" method="POST" data-validate="parsley" class="form parsley-form" data-validate="parsley" enctype="multipart/form-data">
					<div class="col-sm-12">						  
						...
						<div class="form-group">
							...
							{if isset($errorArray.item_type)}<span class="error">{$errorArray.item_type}</span>{else}<span class="smalltext">Type of the feature</span>{/if}					  
						</div>
						<div class="form-group" id="item_cipher_group">
							...
							{if isset($errorArray.item_cipher)}<span class="error">{$errorArray.item_cipher}</span>{else}<span class="smalltext">Code of the feature</span>{/if}					  
						</div>
						<div class="form-group" id="item_variable_group">
							...
							{if isset($errorArray.item_variable)}<span class="error">{$errorArray.item_variable}</span>{else}<span class="smalltext">Type of the feature</span>{/if}					  
						</div>
						<div class="form-group" id="item_name_group">
							<label for="item_name">Name</label>
							<input type="text" id="item_name" name="item_name" class="form-control" value="" />
							{if isset($errorArray.item_name)}<span class="error">{$errorArray.item_name}</span>{else}<span class="smalltext">Name of the feature</span>{/if}					  
						</div>
						<div class="form-group" id="item_value_group">
							...
							{if isset($errorArray.item_value)}<span class="error">{$errorArray.item_value}</span>{else}<span class="smalltext">Value of feature, e.g. HTML code (#DFE83D), size (XXL), etc...</span>{/if}					  
						</div>
                        <div class="form-group" id="item_image_group">
        					...
        					{if isset($errorArray.mediafile)}<span class="error">{$errorArray.mediafile}</span>{else}<span class="smalltext">N.B.: If your media are too big and being cropped, please use the windows software "Paint" or "Microsoft Office Picture Manager" to resize them.</span>{/if}					  
                        </div>						
						<div class="form-group">
